...
class PEGASUSDataset(Dataset):
    """Dataset for PEGASUS summarization/simplification tasks"""
    
    def __init__(self, data: List[Dict], tokenizer, max_length: int = 1024, task: str = "summarization"):
        self.data = data
...
class PEGASUSTrainer:
    """Specialized trainer for PEGASUS models"""
    ...
    def prepare_data(self, data: List[Dict], train_ratio: float = 0.8) -> Tuple:
        ...
        logger.info(f"PEGASUS data split: {len(train_data)} training, {len(val_data)} validation samples")
    ...
    def train(
        ...
        # Create datasets
        train_dataset = PEGASUSDataset(train_data, self.tokenizer, max_length, task)
        val_dataset = PEGASUSDataset(val_data, self.tokenizer, max_length, task)
        ...
        logger.info(f"PEGASUS training completed. Model saved to {output_dir}")
        return metrics
    
    def evaluate(self, test_data: List[Dict], model_path: Optional[str] = None, task: str = "summarization") -> Dict[str, Any]:
        """Evaluate PEGASUS model on test data"""
        if model_path:
        ...
        # Create dataset
        test_dataset = PEGASUSDataset(test_data, tokenizer, task=task)
