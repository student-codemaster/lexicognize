...
class BARTDataset(Dataset):
    """Dataset for BART summarization/simplification tasks"""
...
class BARTTrainer:
    """Specialized trainer for BART models"""
    ...
    def prepare_data(self, data: List[Dict], train_ratio: float = 0.8) -> Tuple:
        ...
        logger.info(f"BART data split: {len(train_data)} training, {len(val_data)} validation samples")
    ...
    def train(
        ...
        # Create datasets
        train_dataset = BARTDataset(train_data, self.tokenizer, max_length, task)
        val_dataset = BARTDataset(val_data, self.tokenizer, max_length, task)
    ...
    def evaluate(self, test_data: List[Dict], model_path: Optional[str] = None, task: str = "summarization") -> Dict[str, Any]:
        ...
        # Create dataset
        test_dataset = BARTDataset(test_data, tokenizer, task=task)
